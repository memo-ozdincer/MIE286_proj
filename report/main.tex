...
\caption{RT vs.\ radial error by condition (no regression lines shown; the relationship is not linear). The scatter shows a Pareto-like boundary: trials cluster in the fast-and-accurate region, with outliers along either the high-error or high-RT axis, but almost no trials are both slow \emph{and} inaccurate. This suggests a performance frontier rather than a simple linear tradeoff.}
\label{fig:sat}
\end{figure}
