...
Figure~\ref{fig:sat} shows the RT-vs-error scatter. The linear correlations are weak ($r_{\text{vis}} = -.193$, $r_{\text{aud}} = -.154$), and visually the relationship is not a clean linear trend. Instead, the scatter shows a boundary pattern: trials cluster in the low-RT/low-error region (fast and accurate), with outliers spreading along either the high-error or high-RT axis, but few trials in the high-RT/high-error quadrant. This suggests a performance boundary rather than a simple linear tradeoff.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{sat_scatter.pdf}
\caption{RT vs.\ radial error by condition. Linear correlations are weak ($r \approx -0.15$ to $-0.19$). The scatter shows a boundary pattern rather than a clean linear SAT. Axes capped at 2000\,ms and 100\,px.}
\label{fig:sat}
\end{figure}

\textbf{H\textsubscript{3} result.} The multiple regression interaction was significant: $\beta_3 = -0.0085$, $t(6759) = -2.28$, $p = .023$. However, the overall model explains very little variance ($R^2 = .029$), meaning this is a statistically detectable but practically small effect. The visual slope ($b = -0.031$) was steeper than auditory ($b = -0.022$), but neither explains the data well. We interpret this cautiously: the linear model captures a real but subtle difference in how speed and error relate across conditions, but a linear SAT is not the dominant pattern in this data.

Figure~\ref{fig:sat_density} shows the same data as a density plot, which better reveals the concentration of trials in the fast-and-accurate region and the boundary structure of the scatter.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{sat_density.pdf}
\caption{2D density plot of RT vs.\ error. Most trials cluster in the low-RT, low-error region. The ``empty'' upper-right quadrant (slow and inaccurate) suggests a performance boundary.}
\label{fig:sat_density}
\end{figure}

\textbf{Per-participant SAT.} The weak population-level correlation masks substantial individual differences. Figure~\ref{fig:pp_cors} shows the distribution of within-participant RT-error correlations. The mean is $r \approx -0.09$ in both conditions, but individual values range from $-0.5$ to $+0.4$. About 68\% of participants show negative correlations (faster = more accurate, the ``easy target'' pattern), while a minority show the classical positive SAT. Roughly 15 participants per condition have $|r| > 0.3$, indicating strong individual tradeoff relationships that cancel out at the population level. This suggests the SAT in this task is an \emph{individual-differences} phenomenon: some people trade off speed for accuracy, others do not, and the population average obscures both groups.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{pp_correlation_dist.pdf}
\caption{Distribution of per-participant Pearson $r$ (RT vs.\ error). Most cluster near zero, but the spread is wide. The SAT is an individual-differences phenomenon in this task.}
\label{fig:pp_cors}
\end{figure}
...
Edge targets (distance from canvas center $\geq 150$\,px) were harder: mean RT of 727\,ms vs.\ 666\,ms for center targets ($p < .001$), with a slightly lower hit rate (94.9\% vs.\ 95.8\%). This confirms a Fitts' Law-like effect: greater cursor travel distance increases response time.
...
\subsection{Post-Miss Behavior}

When participants missed, their behavior changed on the next trial. Figure~\ref{fig:postmiss} shows how RT evolves in the trials following a miss. Both conditions show elevated RT immediately after a miss, but the visual condition shows a larger slowdown: participants were 65.5\,ms slower after a miss under visual feedback vs.\ only 17.4\,ms under auditory. This suggests visual feedback makes failure more salient, causing a stronger corrective response.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{post_miss_rt.pdf}
\caption{RT trajectory following a miss. Trial 0 = immediately after a miss. Visual participants slow down more, suggesting stronger reaction to failure under visual feedback.}
\label{fig:postmiss}
\end{figure}
...
\textbf{Limitations.} Several factors limit interpretation:
\begin{itemize}
  \item \textbf{Weak effects.} H\textsubscript{1} and H\textsubscript{2} were not significant. The 9.7\,ms RT difference and 0.5\,px error difference are small relative to between-participant variability. With $d \approx 0.17$ and $n = 72$, the study was underpowered for these effect sizes (power $\approx 0.30$), increasing Type~II error risk.
  \item \textbf{Near-ceiling accuracy.} Hit rates of $\sim$95\% left little room for error differences to emerge. A harder task (smaller targets, shorter time limits) might reveal larger accuracy effects.
  \item \textbf{Linear SAT model.} The RT-vs-error relationship is not cleanly linear ($R^2 = .029$). The significant interaction ($p = .023$) captures a real but small effect. The boundary pattern in the scatter (few slow-and-inaccurate trials) suggests the SAT in this task operates more as a constraint envelope than a linear function.
  \item \textbf{Convenience sampling.} All participants were 18--24 university students. Results may not generalize to other populations.
  \item \textbf{Device variability.} Mouse users were 178\,ms faster than trackpad users, introducing noise.
  \item \textbf{Feedback salience.} The visual border glow is inherently more perceptually salient than a brief tone. Our comparison is between feedback ``as deployed,'' not pure modality. The motivational effect we observed may partly reflect this salience difference.
  \item \textbf{Leaderboard.} Introduced mid-collection, it increased speed by 96\,ms ($p < .001$). Though accuracy was unchanged and the effect was orthogonal to the within-subject comparison, it may have altered participant motivation.
\end{itemize}
